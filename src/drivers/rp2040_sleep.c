...
static int rp2040_sleep_init(const struct device *dev) {
  ...
  LOG_INF("SCR: %08x, SLEEP_EN0: %08x, SLEEP_EN1: %08x, WAKE_EN0: %08x, "
          "WAKE_EN1: %08x",
          *((uint32_t *)(PPB_BASE + SCR)),
          *((uint32_t *)(CLOCKS_BASE + SLEEP_EN0)),
          *((uint32_t *)(CLOCKS_BASE + SLEEP_EN1)),
          *((uint32_t *)(CLOCKS_BASE + WAKE_EN0)),
          *((uint32_t *)(CLOCKS_BASE + WAKE_EN1)));
...
#define RP2040_SLEEP_DEFINE(n)                                                 \
  ...
  DEVICE_DT_INST_DEFINE(n, rp2040_sleep_init, NULL, NULL, &config##n,          \
