...
static int ip_inc_dec_handle_event(const struct device *dev,
  ...
  for (size_t i = 0; i < cfg->codes_len; i++) {
    ...
    // Cap taps to prevent high-magnitude inputs from flooding the behavior
    // queue
    if (taps > 5) {
      taps = 5;
    }
...
#define IP_INC_DEC_INST(n)                                                     \
  ...
  DEVICE_DT_INST_DEFINE(n, NULL, NULL, NULL, &ip_inc_dec_config_##n,           \
                        POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT,      \
                        &ip_inc_dec_driver_api);
